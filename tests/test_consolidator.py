...
async def test_consolidator_turns_repeated_failures_into_a_rule(consolidator, store):
    """The self-improving loop: failures.md in, an appended rule out."""
    await store.async_setup()
    await store.async_add_memory("some fact", topic="test")
    for _ in range(3):
        await store.async_record_failure(
            "get_statistics", "entity_id='sensor.solar_production_today' -> No long-term statistics"
        )
    ...
    with patch.object(consolidator, "_call_llm", AsyncMock(side_effect=fake_llm)):
        result = await consolidator.async_run()

    assert "Recent tool failures" in captured["prompt"]
    assert "sensor.solar_production_today" in captured["prompt"]
    assert "added 1 rules" in result
    assert "sensor.pv_total" in (store._root / "memories" / "rules.md").read_text()
    assert "Learned from failures" in (store._root / "log.md").read_text()


async def test_consolidator_will_not_flood_rules_with_lessons(consolidator, store):
    await store.async_setup()
    await store.async_add_memory("some fact", topic="test")
    ...
    plan = json.dumps({
        "wiki_updates": [],
        "memories_to_clear": [],
        "rules_to_add": [{"text": f"lesson {i}"} for i in range(4)],
    })
    with patch.object(consolidator, "_call_llm", AsyncMock(return_value=plan)):
        result = await consolidator.async_run()

    assert "Refusing to add 4 rules" in result
    assert not (store._root / "memories" / "rules.md").exists()


async def test_consolidator_does_not_re_add_a_rule_it_already_learned(consolidator, store):
    await store.async_setup()
    await store.async_add_memory("for solar use sensor.pv_total", topic="rules")
    await store.async_add_memory("some fact", topic="test")

    plan = json.dumps({
        "wiki_updates": [],
        "memories_to_clear": [],
        "rules_to_add": [{"text": "for solar use sensor.pv_total"}],
    })
    with patch.object(consolidator, "_call_llm", AsyncMock(return_value=plan)):
        result = await consolidator.async_run()

    assert "added 0 rules" in result
    assert (store._root / "memories" / "rules.md").read_text().count("pv_total") == 1


async def test_consolidator_marks_the_failure_it_learned_from(consolidator, store):
    ...
    plan = json.dumps({
        "wiki_updates": [],
        "memories_to_clear": [],
        "rules_to_add": [{"text": "for solar production use sensor.pv_total"}],
