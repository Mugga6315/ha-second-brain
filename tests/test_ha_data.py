...
def test_resolve_entity_skips_computed_name_alias_sentinel():
    """Regression: an entry's `aliases` can hold the COMPUTED_NAME sentinel
    (AliasEntry = str | ComputedNameType), a truthy non-str. Resolving a name
    against such an entry raised
    `AttributeError: 'ComputedNameType' object has no attribute 'strip'` -
    observed live 2026-08-07 on a get_history call. The sentinel must be
    skipped while real string aliases still resolve."""
    ...
    entry = type(
        "_Entry", (), {
            ...
            "name": None,
            "original_name": None,
            "aliases": {COMPUTED_NAME, "Fake Alias"},
        },
    )()
    reg = type("_Reg", (), {"entities": {entry.entity_id: entry}})()
    h = _resolver_hass()
    with patch(
        "homeassistant.helpers.entity_registry.async_get", return_value=reg
    ):
        # sentinel present but no string matches -> honest passthrough, no crash
        assert _resolve_entity(h, "Nothing Named This") == "Nothing Named This"
        # a real string alias on the same entry still resolves
        assert _resolve_entity(h, "Fake Alias") == "light.fake_entity"


def test_resolve_entity_recovers_slugified_guess():
    """Regression: with only a short alias exposed, the model slugifies it into
    an entity_id that doesn't exist - a short alias "Fake Widget" -> guess
    light.fake_widget, real id light.dev_node_fake_widget - and get_history
    returned "no recorded state changes" (observed live 2026-08-07).
    slugify(name) == object_id recovers the real id, and the domain scope
    blocks a cross-domain collision."""
    real_id = "light.dev_node_fake_widget"
    entry = type(
        "_Entry", (), {
            "entity_id": real_id,
            "name": None,
            "original_name": "dev-node Fake Widget",
            "aliases": {"Fake Widget"},
        },
    )()
    reg = type("_Reg", (), {"entities": {real_id: entry}})()
    h = _resolver_hass()
    with patch(
        "homeassistant.helpers.entity_registry.async_get", return_value=reg
    ):
        # guessed slug from the short alias, right domain -> real id
        assert _resolve_entity(h, "light.fake_widget") == real_id
        # slug from the device-prefixed name -> same id
        assert _resolve_entity(h, "light.dev_node_fake_widget") == real_id
        # same object_id but wrong domain must NOT cross over
        assert (
            _resolve_entity(h, "switch.fake_widget") == "switch.fake_widget"
        )
...
def test_history_includes_state_carried_into_window(hass):
    """Regression: a light on the whole window changed state zero times inside
    it, so with include_start_time_state=False `_history` returned nothing and
    the tool said "no recorded state changes" - observed live 2026-08-07 for a
    light that was 100% on all day. The carried-in state must be included."""
    from custom_components.second_brain.ha_data import _history

    captured = {}

    def fake_scdp(hass_, start, end, entity_id, **kw):
        captured.update(kw)
        return {entity_id: [_FakeState("on", start)]}

    with patch(
        "homeassistant.components.recorder.history.state_changes_during_period",
        fake_scdp,
    ):
        rows = _history(hass, dt_util.now(), dt_util.now(), "light.x")

    assert captured["include_start_time_state"] is True
    assert [s.state for s in rows] == ["on"]
...
async def test_named_day_start_is_that_calendar_day(hass):
    """C-A10: the model must not have to do date arithmetic."""
    from datetime import timedelta

    from homeassistant.util import dt as dt_util

    from custom_components.second_brain.ha_data import GetStatisticsTool

    tool = GetStatisticsTool(hass)
    start, end = await tool._async_range({"start": "yesterday"}, calendar_day=True)

    expected = dt_util.start_of_local_day(dt_util.now() - timedelta(days=1))
    assert start == expected
    assert end - start == timedelta(days=1)


async def test_named_day_today_stops_at_now(hass):
    from datetime import timedelta
    ...
    tool = GetStatisticsTool(hass)
    start, end = await tool._async_range({"start": "today"}, calendar_day=True)

    assert start == dt_util.start_of_local_day(dt_util.now())
    assert end <= dt_util.now() + timedelta(seconds=1)


async def test_future_start_reports_a_range_error(hass):
    """C-A8: the calendar-day branch skipped the start >= end check."""
    from homeassistant.util import dt as dt_util

    from custom_components.second_brain.ha_data import GetStatisticsTool

    tool = GetStatisticsTool(hass)
    future = (dt_util.now() + timedelta(days=30)).strftime("%Y-%m-%d")
    with pytest.raises(ValueError, match="must be before"):
        await tool._async_range({"start": future}, calendar_day=True)


async def test_range_header_names_the_weekdays(hass):
    """The model miscomputes dates; a weekday makes a wrong day audible."""
    from custom_components.second_brain.ha_data import _fmt_range

    start = dt_util.as_local(datetime(2026, 7, 24, 0, 0))
    end = dt_util.as_local(datetime(2026, 7, 25, 0, 0))

    header = _fmt_range(start, end)

    assert "2026-07-24 00:00 (Fri)" in header
    assert "2026-07-25 00:00 (Sat)" in header
