...
# The model is a tool caller, not a calendar. Observed live 2026-07-26: with
# GetDateTime already returning 2026-07-26, "gestern" produced
# start=2026-07-24 end=2026-07-26 - a two-day window starting the day before
# yesterday. The description already told it what to do, so the arithmetic
# moves here. English keywords only: the model translates "gestern" fine, it is
# the date maths it gets wrong.
_NAMED_DAYS = {"today": 0, "yesterday": 1}


def _named_day(value: str) -> datetime | None:
    """Resolve 'today'/'yesterday' to the start of that local calendar day."""
    days = _NAMED_DAYS.get(value.strip().lower())
    if days is None:
        return None
    return dt_util.start_of_local_day(dt_util.now() - timedelta(days=days))
...
def _parse_when(
    ...
    if not value:
        return default
    text = value.strip()
    if (named := _named_day(text)) is not None:
        return named
...
def _fmt_range(start: datetime, end: datetime) -> str:
    """Echo the resolved window with weekday names.

    The model computes dates and gets them wrong - live 2026-07-26 it asked for
    2026-07-24..25 when the user said "gestern" and today was the 26th. Nothing
    server-side can tell a wrong day from an intended one, but a weekday in the
    answer makes the mistake visible to whoever is listening instead of leaving
    a plausible-looking number.
    """
    fmt = "%Y-%m-%d %H:%M (%a)"
    return f"{dt_util.as_local(start).strftime(fmt)} -> {dt_util.as_local(end).strftime(fmt)}"


class _RecorderTool(llm.Tool):
    """Shared plumbing: range parsing and a consistent header line."""

    def __init__(self, hass, record_failure=None) -> None:
        self._hass = hass
        # Optional callback into the store's failure log. Optional on purpose:
        # this module stays standalone and removable (docs/HA_DATA.md), so it
        # must work with nothing wired to it.
        self._record_failure = record_failure

    async def _fail(self, message: str, **context) -> dict:
        """Return a tool error and record it for the nightly pass to learn from."""
    ...
    async def _async_range(
        ...
        whole_day = bool(_BARE_DATE.match(raw_start)) or raw_start.lower() in _NAMED_DAYS
        ...
        if calendar_day and whole_day and not args.get("end"):
            start = _parse_when(raw_start, dt_util.now())
            end = min(start + timedelta(days=1), dt_util.now())
            # C-A8: this branch used to return before the sanity check, so a
            # start in the future produced the misleading "the recorder purged
            # it" error instead of naming the real problem.
            if start >= end:
...
class GetStatisticsTool(_RecorderTool):
    ...
    description = (
        ...
        "the answer comes back computed, with its unit. For yesterday pass "
        "start='yesterday'; for a named date pass that date (2026-07-22). Either "
        "way leave end out - it means that whole calendar day, not the last 24 "
        "hours. Never compute a date yourself. Set breakdown='day' or "
    ...
    async def async_call(
        ...
        try:
            ...
            return await self._fail(str(e), entity_id=args["entity_id"])
        ...
        header = (
            f"{entity_id} | {_fmt_range(start, end)} local"
        )
...
def _resolve_entity(hass, given: str) -> str:
    ...
    # The model often slugifies an exposed name into an entity_id that doesn't
    # exist: "Kitchen Cabinet Light" -> guess light.kitchen_cabinet_light, real
    # id light.msensor_kueche_kitchen_cabinet_light - observed live 2026-08-07.
    # Recover it by matching slugify(name) == object_id, scoped to the same
    # domain so it can't cross over to an unrelated entity.
    domain, _, obj = given.partition(".")

    def _match(entity_id: str, names) -> bool:
        names = [n for n in names if isinstance(n, str)]
        if any(n.strip().lower() == needle for n in names):
            return True
        return bool(obj) and entity_id.startswith(f"{domain}.") and any(
            slugify(n) == obj for n in names
        )

    for state in hass.states.async_all():
        if _match(state.entity_id, (state.attributes.get("friendly_name"),)):
            return state.entity_id
    from homeassistant.helpers import entity_registry as er

    for entry in er.async_get(hass).entities.values():
        if _match(entry.entity_id, (entry.name, entry.original_name, *(entry.aliases or ()))):
...
def _history(hass, start, end, entity_id: str) -> list:
    ...
    # include_start_time_state=True prepends the state carried INTO the window.
    # Without it an entity that never changed inside the window (e.g. a light on
    # the whole time) returns zero rows and reads as "no recorded state changes"
    # - observed live 2026-08-07 for a light that was 100% on all day.
...
async def async_extra_tools(hass, data: dict, record_failure=None) -> list[llm.Tool]:
    ...
    try:
        ...
        return []
    return [
        GetStatisticsTool(hass, record_failure),
